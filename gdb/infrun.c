...
extern pthread_t task_build_handle;
...
handle_signal_stop (struct execution_control_state *ecs)
{
  ...
  /* cngdb : status == synced */
  if (cngdb_current_focus () &&
      cngdb_current_focus ()->state == CNDBG_COORD_SYNC)
    {
      random_signal = true;
    }

  /* check if we hit end, try to auto switch */
  if (random_signal && cngdb_focus_on_device () &&
      (cngdb_current_focus ()->state == CNDBG_COORD_SYNC ||
       cngdb_current_focus ()->state == CNDBG_COORD_FINISHED))
    {
      if (!cngdb_auto_switch ())
        {
          /* don't need auto switch, return back */
          return;
        }
      CNGDB_INFO ("start auto switching coord");
      if (!cngdb_auto_switch_focus ())
        {
          CNGDB_FATAL ("switch focus failed, error in device");
        }
      keep_going (ecs);
      return;
    }
  ...
  if (random_signal)
    {
      ...
      if (cngdb_find_thread_by_ptid (ecs->ptid) != NULL)
        {
          struct cngdb_thread_chain * thread =
            cngdb_find_thread_by_ptid (ecs->ptid);
          /* cngdb: resume from device */
          if (thread->thread_status == CNDBG_THREAD_FINISHED)
            {
              if (cngdb_find_thread_by_status (CNDBG_THREAD_DEBUGGING |
                                               CNDBG_THREAD_PENDING))
                {
                  /* throw exception when there more than 1 thread debugging */
                  CNGDB_FATAL ("exception occured on thread control");
                }
              else if (cngdb_find_thread_by_status (CNDBG_THREAD_WAITING))
                {
                  struct cngdb_thread_chain * waiting_thread =
                    cngdb_find_thread_by_status (CNDBG_THREAD_WAITING);
                  /* start another waiting thread */
                  cngdb_thread_mark (waiting_thread, CNDBG_THREAD_PENDING);
                  /* open thread to build cngdb backend */
                  pthread_create (&task_build_handle, NULL,
                                  cngdb_api_task_build,
                                  (void*)(&(waiting_thread->tinfo->ptid.pid)));
                  usleep (500000);
                  cngdb_nat_resume_thread (waiting_thread->tinfo->ptid);
                  /* assume both thread is running */
                  keep_going (ecs);
                  cngdb_remove_offdevice_thread (ecs->ptid);
                  inferior_ptid = waiting_thread->tinfo->ptid;
                  CNGDB_INFO ("start another waiting thread");
                  return;
                }
              else
                {
          ...
          /* cngdb: keep going when waiting */
          else if (thread->thread_status == CNDBG_THREAD_PENDING)
            {
              CNGDB_INFO ("find waiting");
              keep_going (ecs);
              return;
            }
        }
...
print_stop_location (struct target_waitstatus *ws)
{
  ...
  struct thread_info *tp = inferior_thread ();
  if (cngdb_focus_on_device () &&
      !cngdb_search_state (~CNDBG_SELECT_INITIALIZED) &&
      cngdb_auto_load)
    {
      return;
    }
...
{
  if (cngdb_focus_on_device () && !cngdb_frame_can_print ())
    {
      return;
    }
  struct cleanup *old_chain;
  ...
  if (cngdb_focus_on_device () &&
      ...
      cngdb_auto_load)
    {
      printf_filtered (_("Cngdb auto loaded ngpf.o, stop at pc = 0.\n"));
    }
...
or signalled."),
  ...
  handle_command ("SIGUSR1 nostop noignore pass", 0);
}
